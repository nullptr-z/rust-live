...
//#region 这里，依旧使用了泛型参数 S。未来，如果要支持 WebSocket，或者在 TCP 之上支持 TLS，它都可以让我们无需改变这一层的代码。

// 处理服务器端的某个 accept`接受 到的 socket`插座 的读写
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    async fn send(&mut self, mas: CommandResponse) -> Result<(), KvError> {
        let mut buf = BytesMut::new();
        mas.encode_frame(&mut buf)?;
...
mod tests {
    ...
    async fn client_server_basic_communication_should_work() -> anyhow::Result<()> {
        ...
        // send `Hset`, await response , 发送 value@1
