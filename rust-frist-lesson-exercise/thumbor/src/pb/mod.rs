...
mod abi;
pub use abi::filter::Filter;
pub use abi::resize::SampleFilter;
pub use abi::*;
...
impl Spec {
  ...
  pub fn new_filter(filter: filter::Filter) -> Self {
    Self {
      data: Some(spec::Data::Filter(abi::Filter {
        filter: filter as i32,
      })),
    }
  }
...
mod tests {
  ...
  fn encode_spec_could_be_decoded() {
    ...
    let image_spce = ImageSpec::new(vec![spec1, spec2]);
    let s: String = image_spce.borrow().into();
    assert_eq!(image_spce, s.as_str().try_into().unwrap());
  }
}
